            ...
            <div className="hidden sm:flex items-center gap-8">
               ...
               {!user ? ( <button onClick={()=> setShowUserLogin(true)}  className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition text-white rounded-full">
                    ...
                    <div className='relative group'>
                        ...
                        <li onClick={logout} className='p-1.5 pl-3 hover:bg-primary/10 cursor-pointer'>Logout</li></ul>
                    </div>
                )}
            </div>
<div className='flex item-center gap-6 sm:hidden'>
    ...
    <button onClick={() => open ? setOpen(false) : setOpen(true)} aria-label="Menu" className="">
            ...
            {/* Mobile Menu */}
           { open && (
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 z-30 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
